...
public class AdminTrainControllerTests
{
    ...
    public async Task Create_Returns201()
    {
        ...
        svc.Setup(s => s.CreateTrainAsync(It.IsAny<CreateTrainRequest>(), It.IsAny<CancellationToken>()))
           .ReturnsAsync(MakeTrain(1));
        var controller = new AdminTrainController(svc.Object);

        var result = await controller.Create(new CreateTrainRequest { TrainName = "T", TrainNumber = "1", Source = "A", Destination = "B" }, CancellationToken.None);
    ...
    {
        var svc = new Mock<ITrainService>();
        svc.Setup(s => s.UpdateTrainAsync(1, It.IsAny<UpdateTrainRequest>(), It.IsAny<CancellationToken>()))
           .ReturnsAsync(MakeTrain(1));
        var controller = new AdminTrainController(svc.Object);

        var result = await controller.Update(1, new UpdateTrainRequest { TrainName = "Updated" }, CancellationToken.None);
    ...
    {
        var svc = new Mock<ITrainService>();
        svc.Setup(s => s.UpdateSeatAvailabilityAsync(1, It.IsAny<UpdateSeatAvailabilityRequest>(), It.IsAny<CancellationToken>()))
           .ReturnsAsync(new SeatAvailabilityDto { Id = 1, TrainId = 1, Date = "2025-06-01", AvailableSeats = 50 });
        var controller = new AdminTrainController(svc.Object);

        var result = await controller.UpdateSeatAvailability(1, new UpdateSeatAvailabilityRequest { Date = "2025-06-01", AvailableSeats = 50 }, CancellationToken.None);
