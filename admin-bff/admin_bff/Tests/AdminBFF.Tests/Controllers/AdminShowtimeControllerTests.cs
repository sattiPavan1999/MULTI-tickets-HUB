...
public class AdminShowtimeControllerTests
{
    ...
    public async Task GetShowtimes_Returns200WithList()
    {
        ...
        svc.Setup(s => s.GetShowtimesAsync(1))
    ...
    {
        var svc = new Mock<IMovieService>();
        svc.Setup(s => s.CreateShowtimeAsync(It.IsAny<CreateShowtimeRequest>()))
        ...
        var result = await controller.CreateShowtime(1, request);
    ...
    {
        var svc = new Mock<IMovieService>();
        svc.Setup(s => s.DeleteShowtimeAsync(1)).Returns(Task.CompletedTask);
        var controller = new AdminShowtimeController(svc.Object);
